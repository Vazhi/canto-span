#!/usr/bin/env python3
"""AA82 high-recall HKCanCor profile for 邊度 and adjacent 邊 + 度.

The query inventories surface forms only. It does not infer location, goal,
source, headless, fragment, continuation, or construction interpretations.
"""
...
CONSTRUCTION = {
    "constructionUuid": "4593e90d-b923-54af-a9ac-533409c38355",
    "permanentCode": "AA82",
    "canonicalIdentity": "BinDouWhPlaceQuestion",
    "legacyRuntimeLabel": "LocativeWhQuestion",
}
QUERY_ID = "AA82-HKCANCOR-BIN-DOU-R1"
PROFILE_KEY = "bin-dou"
ADJACENT_BIN_DOU = "adjacent_bin_dou_tokens"
SINGLE_TOKEN_BIN_DOU = "single_token_bin_dou"
...
def token_predicate(tokens: Sequence[object], token_index: int) -> Match | None:
    ...
    if word == "邊度":
        ...
        segmentation_profile = SINGLE_TOKEN_BIN_DOU
    elif (
        word == "邊"
        and token_index + 1 < len(tokens)
        and tokens[token_index + 1].word == "度"  # type: ignore[attr-defined]
    ):
        end_index = token_index + 2
        segmentation_profile = ADJACENT_BIN_DOU
    else:
        return None
...
def summary_builder(
    ...
    return {
        ...
        "endpoint": (
            "Every exact HKCanCor token 邊度 and every exact adjacent 邊 + 度 "
        ...
        "query": {
            "surfaceProfiles": [ADJACENT_BIN_DOU, SINGLE_TOKEN_BIN_DOU],
            "selectionUnit": (
                "exact token or adjacent token sequence within one utterance"
            ),
            "syntacticPositionRestricted": False,
            "governingPredicateRestricted": False,
            "questionParticleRequired": False,
            "placeRoleInferred": False,
            "semanticSelectionPerformed": False,
            "overlapPolicy": (
                "The scanner anchors one candidate at 邊 or 邊度; the 度 token "
        ...
        "stableIdPolicy": {
            "namespace": QUERY_ID,
            "prefix": "aa82-",
            "anchorToken": "邊 or 邊度 at the source location",
        },
        "interpretationWarning": (
            "The inventory deliberately retains location, motion-goal, source, "
            "headless, continuation, fragment, repaired, and annotation-boundary "
            "candidates. Expert context review must determine AA82 membership and "
            "interpretation. Counts do not establish evidence, productivity, "
...
QUERY_PROFILES = {
    PROFILE_KEY: QueryProfile(
        ...
        candidate_id_prefix="aa82-",
        inventory_json="hkcancor-aa82-bin-dou-candidate-inventory.json",
        inventory_tsv="hkcancor-aa82-bin-dou-candidate-inventory.tsv",
        summary_json="hkcancor-aa82-bin-dou-query-summary.json",
        construction=CONSTRUCTION,
...
def main() -> None:
    profile_cli(
        ...
        description="Generate or check the deterministic AA82 HKCanCor query.",
    )
