...
const outputPath = path.join(root, "validation", "native-speaker-naturalness-v0.5.165.json");
...
const statuses = new Set(patterns.map((row) => row.status));
check("native screening did not promote any productive status", !statuses.has("supported_productive") && !statuses.has("supported_lexicalized"));
check("acceptable surface mapping is not analysis validation", patterns.filter((row) => row.native_naturalness_acceptable_count > 0).every((row) => row.native_validation_scope.includes("NOT_") && row.native_validation_scope.includes("ANALYSIS_VALIDATION")));
check("rejected mapped surfaces remain review-bearing", patterns.filter((row) => row.native_naturalness_rejected_count > 0).every((row) => row.native_conflict_disposition !== "none"));
...
check("PerfectiveVP remains provisional, not validated", patterns.find((row) => row.pattern === "PerfectiveVP")?.status === "provisional");
...
const output = {
  schema: "canto-span-native-speaker-naturalness-audit-result-v1",
  runtime_version: "0.5.165",
