...
const result = { runtime_version: api.runtimeVersion, registry_owner: "grammar/active/*.md + grammar/archived/*.md", status: (!missing.length && !unknown.length && !mismatched.length && active.size === 169 && retiredSet.size === 12) ? "PASS" : "FAIL", runtime_registry_labels: api.labels.length, active_construction_notes: active.size, retired_labels: retiredSet.size, status_counts: counts, missing_active_labels: missing, unclassified_runtime_labels: unknown, runtime_status_mismatches_not_applicable: mismatched };
