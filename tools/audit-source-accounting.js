...
const result = { schema: "canto-span-source-accounting-audit-v6", checkpoint: `v${api.runtimeVersion}-current`, runtime_registry_labels: runtime.size, construction_notes: noteMap.size, retired_labels: retiredSet.size, total: checks.length, passed: checks.length - failures.length, failed: failures.length, status: failures.length ? "FAIL" : "PASS", checks, failures };
