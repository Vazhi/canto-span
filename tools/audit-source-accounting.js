...
check("runtime registry has 168 labels", runtime.size === 168, String(runtime.size));
check("construction notes have 168 labels", noteMap.size === 168, String(noteMap.size));
check("runtime exactly equals construction notes", runtime.size === noteMap.size && [...runtime].every((x) => noteMap.has(x)));
check("thirteen labels are archived as retired", retiredSet.size === 13, String(retiredSet.size));
...
const result = { schema: "canto-span-source-accounting-audit-v6", checkpoint: "v0.5.194-speech-transfer-complement-wrapper-audit", runtime_registry_labels: runtime.size, construction_notes: noteMap.size, retired_labels: retiredSet.size, total: checks.length, passed: checks.length - failures.length, failed: failures.length, status: failures.length ? "FAIL" : "PASS", checks, failures };
