...
check("status counts match v0.5.194 speech/transfer/complement checkpoint", Object.entries(expectedCounts).every(([status, count]) => counts[status] === count) && Object.keys(counts).length === Object.keys(expectedCounts).length, JSON.stringify(counts));
