...
// Add only explicitly reviewed surface crosswalks. R1-R6 deliberately did not
// infer broad Traditional/Simplified or colloquial spelling equivalence.
...
function classify(record, runtime) {
  ...
  if (variant && runtime.direct.has(variant)) {
    return { ...record, status: "covered_variant", note: `verified_variant_of=${variant}`, runtimeModule: "-", runtimeJyutping: "-", runtimeMeta: "-" };
  }

  if ((record.jyutping || "").startsWith("*") || record.definition === "NO DEF") {
    return { ...record, status: "manual_review", note: "cifu_generated_or_undefined", runtimeModule: "-", runtimeJyutping: "-", runtimeMeta: "-" };
  }
  ...
  if (segments) {
    return {
      ...
      note: `lexically_segmentable_as=${segments.join("+")};not_proof_of_construction_analysis`,
      runtimeModule: "-",
      runtimeJyutping: "-",
      runtimeMeta: "-",
    };
...
function renderTsv(rows) {
  ...
  for (const row of rows) {
    lines.push([
      ...
      row.status, row.note, row.runtimeModule, row.runtimeJyutping, row.runtimeMeta,
    ].map(clean).join("\t"));
  }
  return `${lines.join("\n")}\n`;
}
...
if (CHECK_ONLY) {
  ...
  if (existing !== output) {
    throw new Error("Top-2000 lexical audit drift: regenerate data/lexical-frequency/cifu-spoken-top-2000.tsv");
  }
} else {
