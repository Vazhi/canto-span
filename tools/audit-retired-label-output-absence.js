...
const np=JSON.parse(fs.readFileSync(path.join(root,"test-data","np-subsystem-v0.5.184.json"),"utf8"));
...
const result={schema:"canto-span-retired-label-output-absence-v2",checkpoint:"v0.5.184-np-subsystem-r1",runtime_version:api.runtimeVersion,retired_labels:[...retiredSet].sort(),case_count:analyzed,active_registry_residue:registryResidue,retired_label_output_hits:hits.length,status:!hits.length&&!registryResidue.length?"PASS":"FAIL",hits};
const out=path.join(root,"validation","v0.5.184");fs.mkdirSync(out,{recursive:true});fs.writeFileSync(path.join(out,"retired-label-output-absence.json"),JSON.stringify(result,null,2)+"\n");console.log(JSON.stringify(result,null,2));if(result.status!=="PASS")process.exit(1);
