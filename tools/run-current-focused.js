...
check("168 construction notes", notes.length === 168, String(notes.length));
...
check("research_pending 60", counts.research_pending === 60, String(counts.research_pending));
...
check("Definition of Done controls supported_productive", /eligible for `supported_productive`[\s\S]*only when all/i.test(dod));
check("Definition of Done controls provisional", /Definition of Done: `provisional`/i.test(dod));
check("Definition of Done requires role-neutral item-level panel thresholds", /30 usable[\s\S]*critical item/i.test(dod) && /no respondent receives special status/i.test(dod));
