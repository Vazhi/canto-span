...
function lockParts(lock) {
  return String(lock || "").split(/[:/]/u).filter(Boolean);
}
...
function validatePortfolioRouting(routing) {
  ...
  if (routing.kind === "research" && !RESEARCH_MODES.has(routing.research_mode)) {
    errors.push("kind=research requires a research_mode");
  }
