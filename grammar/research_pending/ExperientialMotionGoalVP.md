...
- Current action: retain the source-linked `去 + 過 + overt goal` profile
...
The runtime is narrowed to attested `去 + 過 + overt goal` material. A
source-linked positive preserves `去過美國` inside its independently parsed
negative-experiential statement; perfective `去咗` and non-motion `見過`
provide executable boundaries. Other motion predicates and goal types remain
research questions.
