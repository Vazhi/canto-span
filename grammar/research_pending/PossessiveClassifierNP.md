...
- Current action: retain the source-linked overt possessor + classifier + noun profile
...
- Implementation-only reachability: `NOMWRAP-002` with `佢整冧咗我間屋。`
- The wrapper is observed as a nested result-frame object; standalone `我間屋。` does not preserve `PossessiveClassifierNP` in complete output.
- The probe's linguistic evidence weight is **0** and does not establish general possessive-classifier productivity.

## v0.5.214-r1 source/runtime reconciliation

The runtime no longer depends on a result-frame wrapper. It recognizes the
source-linked overt `possessor + classifier + noun` profile directly, with
focused positives for `我架車` and `佢本書`.

Executable boundaries keep `possessor + 嘅 + noun` and classifier-less
possessor–noun adjacency outside this node. Full-NP possessors, demonstratives,
headless readings, number interpretation, and broader classifier productivity
remain unresolved.
