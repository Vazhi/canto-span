...
- Current action: retain the source-linked `返 + 上 + 嚟/去` profile
...
- Standard executable test file: `tests/constructions/CompoundDirectionalMotionVP.json`
...
## v0.5.214-r3 source/runtime reconciliation

The runtime retains the independently attested three-part directional strings
`返上嚟` and `返上去`. Executable boundaries distinguish two-part `上嚟` and
the manner-plus-directional predicate `行返過嚟`; the latter must not be
collapsed into the same whole-span node.

Other direction/deictic combinations, object placement, manner-verb
attachment, and productive constituency remain unresolved.
