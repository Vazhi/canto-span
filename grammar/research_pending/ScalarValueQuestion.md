...
- Current action: retain separately typed lexical-price and scalar-dimension question profiles
...
- Implementation-only reachability: `ESQWRAP-005` with `幾錢呀？`
- Protects the exact price-question route only. It does not establish a single construction across heterogeneous scalar domains.
...
## v0.5.214-r4 source/runtime reconciliation

The runtime retains two explicitly distinguished source-linked profiles:

- lexical price `幾錢` with an optional final particle;
- compositional question-use `幾 + licensed scalar-dimension predicate`.

The former optional quantity/person/approximation cross-product around `幾錢`
is removed. Executable boundaries keep that unsourced expansion and ordinary
non-question degree-stative predication outside this node. Broader lexical
selection, spoken-question licensing without punctuation, and whether the two
profiles should eventually receive separate public labels remain unresolved.
