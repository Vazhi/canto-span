# Issue #443 pre-audit research: subjectless `用嚟 VP` licensing
...
The interim pilot suggests that bare `用嚟 VP` strings are often accepted when respondents can recover or accommodate a referent. That result is used only as a research trigger. It does not establish that the surface is an autonomous complete clause, identify a unique hidden syntactic argument, or authorize parser reconstruction.

This packet separates four evidence types:

1. direct descriptions of `用嚟` and instrumental/purposive structures;
2. Cantonese descriptions of topicalization and omitted arguments;
3. general Chinese and cross-linguistic fragment analyses;
4. hypotheses requiring controlled context, prosody, and final survey evidence.

## Source-supported starting point

Wong (2023) describes instrumental `用嚟` and states that the instrument may be omitted or placed in subject position. Cheung (2018) treats `用來／用嚟` as a conventionalized purposive expression. Matthews and Yip document Cantonese topic chains and omitted pronouns, while Lee (2018) establishes several discourse-sensitive Cantonese object-omission profiles.
...
| Analysis | Structural/discourse sketch | Current evidence state |
| --- | --- | --- |
...
| Repair or failed parse | Speaker asks what is used, inserts a noun phrase, or rejects the string | Must remain a genuine negative outcome rather than being recoded as implicit reference. |

## Context contract to test

A later controlled study must distinguish at least the following:

- immediate unique antecedent;
