...
| `SRC-WORDSHK-ZOEK6-POLYFUNCTION` | `LEXICAL_OR_PRONUNCIATION_ONLY` | `entry_inspected` | 粵典, `着／著 zoek6`, entries for verbal suffix/result, contact or attachment, achieved state, sleep-result, and chess-move or strategy senses. <https://words.hk/zidin/%E7%9D%80> | Directly documents non-wear `zoek6` lexical and grammatical profiles, including result/correctness-like suffixal use and chess/strategy `一着`. | One dictionary entry inventories distinct senses; it does not prove that they form one construction or settle every syntactic analysis. | `RETAIN_AS_SEPARATE_ZOEK6_PROFILE_FAMILY` |
| `SRC-WORDSHK-ZOEK6-IGNITION` | `LEXICAL_OR_PRONUNCIATION_ONLY` | `entry_inspected` | 粵典, `着火／著火 zoek6 fo2`, ‘catch fire; be on fire’, with contextual examples. <https://words.hk/zidin/%E7%9D%80%E7%81%AB> | Directly supports an ignition lexical compound under `zoek6`, separate from the wear verb. | Supports the compound and reading only; it does not authorize a generic ignition construction or character rule. | `RETAIN_AS_SEPARATE_LEXICAL_COLLISION` |
| `SRC-WORDSHK-ZOEK6-GAN2-COLLISION` | `LEXICAL_OR_PRONUNCIATION_ONLY` | `entry_inspected` | 粵典, `着緊／著緊 zoek6 gan2`, ‘care about; be concerned about’. <https://words.hk/zidin/%E7%9D%80%E7%B7%8A> | Establishes a lexical compound whose written form collides with wear `着 zoek3 + 緊 gan2`. | Does not resolve every `著緊` token automatically; context and reading remain necessary. | `RETAIN_AS_SEPARATE_LEXICAL_COLLISION` |
| `SRC-WORDSHK-ZOEK3-DURATIVE-EXAMPLES` | `ATTESTATION_ONLY` | `examples_inspected` | 粵典 contextual example pages containing `著住對襪`, `著住件破舊嘅大褸`, and `幫主人著衫`. Example-bearing entries include <https://words.hk/zidin/%E5%B0%8D> and <https://words.hk/zidin/%E4%B8%AB%E9%AC%9F>. | Attests wear `著／着 zoek3` with durative `住`, clothing objects, and an outer help/benefactive context. | Examples occur inside entries for other words and do not independently define a construction or broad distribution. | `RETAIN_AS_CONTEXTUAL_ATTESTATION` |
...
5. Direct `zoek6` entries separately document suffixal/result, attachment or achieved-state, ignition, lexical-compound, and chess/strategy profiles.
6. Written `著緊／着緊` has at least one lexical `zoek6 gan2` ‘care about’ analysis distinct from `zoek3 + gan2` wear structure.
7. `著` and `着` cannot be treated as an unconditional `zoek3` alias pair.
