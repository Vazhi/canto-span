...
The paired-clause relation map reviews PRQ2-008–015 and PRQ2-033–035 as eleven separate families across 151 checked-in collision rows. The three bounded typed cores in #430 have separate reserved candidate UUIDs for overt sufficient condition, ordered preference, and clausal necessary condition. The reopened #431 blocker audit withdraws four branch-only UUID proposals for PRQ2-010, PRQ2-011, PRQ2-012, and PRQ2-033: all four ledgers remain ungraded debt, three are flagged known weak cores, PRQ2-012 required recovery from an earlier lexical-category-only source warning, and the proposed PRQ2-033 profile omitted directly described negative `又…又`. Fresh v0.5.218 probes confirm the runtime gap still exists, but no shared implementation batch is authorized. #431 remains open for graded source recovery, arity, polarity, omission, continuation, and discourse-boundary decisions. Three evidence-constrained families remain in #432, and PRQ2-015 remains research-only through #409. Generic clause graphs remain parser infrastructure with zero independent linguistic-evidence weight.
